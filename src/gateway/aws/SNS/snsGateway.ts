...
import WarehouseEvent from '../../../domain/WarehouseEvent';
...
async function publishMessageToTopic(event: WarehouseEvent) {
  ...
  try {
    ...
    logger.error(`Error publishing event with id (${event.id}) to SNS WarehouseServiceTopic. Error: ${error.message}: ${error.stack}`);
